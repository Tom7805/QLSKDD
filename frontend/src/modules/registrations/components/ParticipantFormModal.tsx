...
export default function ParticipantFormModal({ open, participant, onClose, onSaved, onError }: Props) {
  ...
  const input = (name: keyof Form) => `mt-1.5 h-11 w-full rounded-xl border px-3.5 text-sm outline-none focus:ring-4 ${errors[name] ? 'border-red-400 focus:ring-red-100' : 'border-slate-300 focus:border-blue-500 focus:ring-blue-100'}`;
