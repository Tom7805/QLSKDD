        ...
        {error ? (
          ...
          <>
            <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:max-w-xs">
              ...
              <p className="mt-2 text-3xl font-bold text-slate-900">{averageRate}%</p>
            </div>
              ...
              <table className="min-w-full text-left text-sm">
                <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="w-64 px-4 py-3">Tỷ lệ tham dự</th>
                  </tr>
                </thead>
                  ...
                  {events.map((event) => (
                    <tr key={event.id}>
                      ...
                      <td className="px-4 py-3">
                        {event.attendanceRate !== null ? <AttendanceRateBar rate={event.attendanceRate} label="" /> : <span className="text-slate-400">—</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
