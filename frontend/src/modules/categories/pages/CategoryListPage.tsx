    ...
    <div className="min-h-full bg-scene p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button type="button" onClick={openCreateForm} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-ink px-4 text-sm font-semibold text-white shadow-sm hover:bg-ink-soft raise">
            <span className="text-xl leading-none" aria-hidden="true">＋</span> Thêm loại mới
          </button>
        </div>

        <div className="overflow-hidden glass-card border-violet-200/80">
                ...
                {categories.map((category, index) => (
                  <article key={category.id} className="border-b border-slate-200 px-4 py-4 last:border-b-0">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button type="button" onClick={() => openEditForm(category)} className="min-w-[120px] rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-ink hover:bg-slate-50 raise">Sửa</button>
